...
class Controller extends BaseController {
    ...
    protected function _get_ptriggers() {
        $rows = $this->db->pq("SELECT pt.propertytriggerid, pt.propertyid, pt.value, pt.comparator, pt.propertyprofileid, pt.scheduleid, pt.schedulestatus, pt.active, p.friendlyname as propertyname, pp.name as propertyprofile, s.name as schedule
    ...
    protected function _add_ptrigger() {
        ...
        $this->db->pq("INSERT INTO propertytrigger (propertyid, value, comparator, propertyprofileid, scheduleid, schedulestatus) 
            VALUES (:1, :2, :3, :4, :5, :6)",
            [$this->args->value('propertyid'), $this->args->value('value'), $this->args->value('comparator'), $this->args->value('propertyprofileid', null), $this->args->value('scheduleid', null), $this->args->value('schedulestatus', null)]);
    ...
    protected function _patch_ptrigger() {
        ...
        foreach (['propertyid' => false, 'value' => false, 'comparator' => false, 'propertyprofileid' => true, 'scheduleid' => true, 'schedulestatus' => true, 'active' => false] as $k => $empty) {
